...
daily <- get_daily_visits(weekly_0525_LA) %>%
  # bind three datasets together
  rbind(get_daily_visits(weekly_0601_LA)) %>%
  rbind(get_daily_visits(weekly_0608_LA)) %>%
...
hourly <- get_hourly_visits(weekly_0525_LA) %>%
  # bind 3 datasets together
  rbind(get_hourly_visits(weekly_0601_LA)) %>%
  rbind(get_hourly_visits(weekly_0608_LA)) %>%
...
# Read in data
# POI / open hours
rate <- read_csv("infection_rate/POI_comm_rates_0624.csv")   # will be eliminated later
poi <- read_csv("SafeGraph/core_places/poi_extended.csv")
open_hours <- read_csv("SafeGraph/core_places/poi_hour_imputed.csv")

# Joining the data
# (will be modified after we include the infection rate data in this script)
risk <- open_hours %>%
  # Join the poi data
  left_join(poi %>% select(safegraph_place_id, location_name, top_category, latitude, longitude, street_address, area_square_feet), 
  ...
  ungroup() %>%
  # Join infection rate data
  left_join(rate %>% select(safegraph_id, `infection%`), by = c("safegraph_place_id" = "safegraph_id")) %>%
  rename(infection_rate = `infection%`) %>%
